...
export function InvoiceList({ invoices }: { invoices: Invoice[] }) {
  ...
  // AR Aging from pending invoices
  ...
  return (
    <>
      ...
      {/* Filter tabs — client-side, instant */}
      <div className="flex gap-2 overflow-x-auto">
        {STATUS_OPTIONS.map((opt) => (
          ...
          >
            {opt.label} ({opt.value === "all" ? invoices.length : invoices.filter((i) => i.status === opt.value).length})
          </button>
        ))}
      </div>
                  ...
                  filtered.map((inv) => (
                    <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3 text-navy-dark">
                        {inv.doctors?.full_name || inv.doctor_id}
                      </td>
