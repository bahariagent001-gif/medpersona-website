...
export default async function ContentPage({
  ...
  const statusBadge: Record<string, "success" | "warning" | "danger" | "info" | "secondary"> = {
    ...
    scheduled: "info", posted: "success", rejected: "danger", failed: "danger",
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/konten/kalender" className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-navy-dark hover:bg-gray-50">
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(statusCounts).sort((a, b) => b[1] - a[1]).map(([status, count]) => (
          <Link
            key={status}
            href={`/konten?status=${status}`}
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              params.status === status ? "bg-teal-dark text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            }`}
          >
        ...
        {params.status && (
          <Link href="/konten" className="rounded-full px-3 py-1 text-xs font-medium text-gray-400 hover:text-gray-600">
            Hapus filter
          </Link>
        )}
      </div>
              ...
              {content?.map((item) => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${platformColors[item.platform] || "bg-gray-100 text-gray-700"}`}>
                      {item.platform}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{item.content_type || "-"}</td>
                  <td className="px-4 py-3 text-gray-500">{item.planned_date ? formatDate(item.planned_date) : "-"}</td>
                  <td className="px-4 py-3 text-center">
                    <Badge variant={statusBadge[item.status] || "secondary"}>{item.status.replace("_", " ")}</Badge>
                  </td>
                </tr>
              ))}
              {(!content || content.length === 0) && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">Tidak ada konten</td></tr>
              )}
            </tbody>
          </table>
        </CardContent>
